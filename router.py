...
VALID_ROUTES = ("web_photo", "map", "diagram", "chart", "illustration", "skip")
AI_ROUTES = ("map", "diagram", "chart", "illustration")  # AI生成班が担当
...
2. map … 地理的位置・国境・領土・移動経路・地名どうしの関係
   例:「ソ連は14か国と国境を接していた」「シベリア鉄道が東西を結んだ」
3. diagram … 概念・仕組み・因果関係・対比・フロー（数値が主役でないもの）
   例:「大陸国家と海洋国家では戦略が異なる」「三権分立の仕組み」
4. chart … 数値・統計・割合・推移・比較データが主役
   例:「軍事費はGDP比6.3%に達した」「人口は3億人を超えた」
5. illustration … 抽象的シーン・比喩・心情・一般的な描写（実在の特定対象ではない）
   例:「緊張が静かに高まっていった」「希望の光が差した」
6. skip … 接続詞・挨拶・問いかけ・内容のない繋ぎ（視覚化する意味がない）
   例:「では、見ていきましょう」「皆さんこんにちは」「次の章へ進みます」

【判定のコツ】
- 実在の固有名詞（人名・事件名・建造物）が主役 → web_photo を優先
- 「本物が見たい」より「概念を説明したい」なら diagram
- 数字が主役なら chart
